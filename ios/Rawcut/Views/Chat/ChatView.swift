...
struct ChatView: View {
    ...
    private func renderScript(_ script: ScriptResponse) {
        ...
        messages.append(ChatMessage(text: "Creating project and starting render...", isUser: false))
        isLoading = true

        Task {
            do {
                // 1. Create project from script title
                ...
                // 2. Set clips from script segments
                ...
                let clips = script.segments.enumerated().map { i, seg in
                    ClipItem(
                        asset_id: seg.clip_id,
                        position: i,
